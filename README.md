...
| GRC | available, invite-only | Many customers, each with a folder tree (up to 8 levels) and many assessments per fiscal year — copy a previous year forward or start blank, then compare two years side by side. One risk register per customer, shared across its assessments. **ISO/IEC 27001:2022**: all 93 Annex A controls with status, owner, justification and evidence links; gap assessment by theme; 5×5 risk register with treatment and residual scoring; Statement of Applicability with CSV export. **NIST CSF 2.0**: Current/Target on a 0–10 scale for all 106 subcategories, gap analysis by Function and Category, Function ratings, Tiers, NIST Implementation Examples, cross-referenced to an ISO 27001 assessment with suggestions, CSV export. Every approved account can see and edit every customer — there is no per-customer access control yet. |
