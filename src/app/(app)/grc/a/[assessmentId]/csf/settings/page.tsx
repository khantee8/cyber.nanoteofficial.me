...
import { getCsfProfile, getCsfScores, getStatuses, listAssessments, toScoreRows } from '@/lib/grc/queries';
...
export default async function CsfSettings({ params }: PageProps<'/grc/a/[assessmentId]/csf/settings'>) {
  const { assessmentId } = await params;
  const { lang, assessment, customer } = await loadAssessment(assessmentId, 'nist-csf-2');
  // Same source rule as prefillFromIso: no ISO assessment → nothing to prefill.
  const isoSourceId = pickIsoSource(await listAssessments(customer.id), assessment.fiscalYear);
  const [profile, scores, statuses] = await Promise.all([getCsfProfile(assessment.id), getCsfScores(assessment.id), isoSourceId ? getStatuses(isoSourceId) : []]);
    ...
    <div className="grid gap-6 lg:grid-cols-12">
      <div className="flex flex-col gap-6 lg:col-span-7">
        <section className="panel p-5">
          ...
          <CsfProfileForm assessmentId={assessment.id} profile={profile} lang={lang} />
        </section>
        <section className="panel p-5">
          ...
          <PrefillButton assessmentId={assessment.id} count={plan.length} lang={lang} />
