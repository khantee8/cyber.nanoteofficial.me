...
export default async function ProfilePage(props: PageProps<'/grc/a/[assessmentId]/csf/profile'>) {
  const [{ assessmentId }, params] = await Promise.all([props.params, props.searchParams]);
  const { lang, assessment, base } = await loadAssessment(assessmentId, 'nist-csf-2');
  const CSF_BASE = `${base}/csf`;
  const rows = await getCsfScores(assessment.id);
      ...
      {CSF_CATEGORIES.filter((c) => CSF_SUBCATEGORIES.some((s) => s.category === c.id && visibleIds.has(s.id))).map((c) => (
        <section key={c.id} className="panel overflow-x-auto">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-line px-4 py-2.5">
            ...
            <BulkTarget assessmentId={assessment.id} categoryId={c.id} lang={lang} />
          </div>
              ...
              {CSF_SUBCATEGORIES.filter((s) => s.category === c.id && visibleIds.has(s.id)).map((s) => {
                ...
                return (
                  <tr key={s.id} className={`align-top hover:bg-surface-2/60 ${r?.inScope === false ? 'opacity-60' : ''}`}>
                    ...
                    <td className="px-2 py-2"><ScorePicker assessmentId={assessment.id} key={`${s.id}:current:${r?.current ?? ''}`} subcategoryId={s.id} field="current" value={r?.current ?? null} lang={lang} /></td>
                    <td className="px-2 py-2"><ScorePicker assessmentId={assessment.id} key={`${s.id}:target:${r?.target ?? ''}`} subcategoryId={s.id} field="target" value={r?.target ?? null} lang={lang} /></td>
