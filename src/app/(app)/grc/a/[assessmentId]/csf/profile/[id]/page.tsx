...
import { assessmentBase, loadAssessment } from '@/lib/grc/context';
import { getCsfScore, getRisks, getStatuses, listAssessments } from '@/lib/grc/queries';
...
export default async function SubcategoryPage({ params }: PageProps<'/grc/a/[assessmentId]/csf/profile/[id]'>) {
  ...
  const isoSourceId = pickIsoSource(await listAssessments(customer.id), assessment.fiscalYear);
  const isoBase = isoSourceId ? `${assessmentBase(isoSourceId)}/iso` : null;
  const [row, statuses, risks] = await Promise.all([getCsfScore(assessment.id, id), isoSourceId ? getStatuses(isoSourceId) : [], getRisks(customer.id)]);
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
      <div className="lg:col-span-7">
        ...
        <div className="panel mt-6 p-5">
          <CsfScoreForm key={id} assessmentId={assessment.id} subcategoryId={id} initial={row} lang={lang} />
        </div>
        {sub.examples.length ? (
      ...
      <aside className="flex flex-col gap-4 lg:col-span-5">
        <SuggestionPanel assessmentId={assessment.id} subcategoryId={id} suggestion={suggestion} current={row?.current ?? null} lang={lang} base={isoBase} />
          ...
          {sub.iso27001.length === 0 ? <p className="text-[12.5px] text-muted-soft">{t(lang, 'csf.iso.none')}</p> : (
            <ul className="divide-y divide-line">
              {sub.iso27001.map((cid) => (
                <li key={cid}>
                  {isoBase ? (
                    <Link href={`${isoBase}/controls/${cid}`} className="flex items-center gap-3 py-2 text-[13px] hover:text-accent">
          ...
          {linked.length === 0 ? <p className="text-[12.5px] text-muted-soft">{t(lang, 'grc.control.noLinked')}</p> : (
            <ul className="divide-y divide-line">
              {linked.map((r) => {
                ...
                return (
                  <li key={r.id}>
                    <Link href={`${customerBase}/risks/${r.id}`} className="flex items-center gap-3 py-2 text-[13px] hover:text-accent">
