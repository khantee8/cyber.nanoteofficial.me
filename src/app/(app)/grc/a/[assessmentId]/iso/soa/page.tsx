...
export default async function SoaPage(props: PageProps<'/grc/a/[assessmentId]/iso/soa'>) {
  const [{ assessmentId }, params] = await Promise.all([props.params, props.searchParams]);
  const print = params.print === '1';
  const { lang, assessment, customer, base } = await loadAssessment(assessmentId, 'iso27001');
  const BASE = `${base}/iso`;
    ...
    <div className={`flex flex-col gap-5 ${print ? 'print:text-black' : ''}`}>
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div className="max-w-2xl">
          ...
          <p className="mt-1 text-[13.5px] leading-relaxed text-muted">{print ? `${customer.name} · ${assessment.title} · ${t(lang, 'grc.soa.generated')} ${generated}` : t(lang, 'grc.soa.lede')}</p>
          {assessment.scope ? <p className="mt-2 text-[12.5px] leading-relaxed text-muted"><span className="text-muted-soft">{t(lang, 'grc.org.scope')}:</span> {assessment.scope}</p> : null}
        </div>
        <div className="no-print flex items-center gap-2">
          <Link href={`${BASE}/soa?print=1`} className="btn">{t(lang, 'grc.soa.print')}</Link>
          <a href={`/api/grc/a/${assessment.id}/soa.csv`} className={`btn btn-primary ${missingJustification.length ? 'pointer-events-none opacity-50' : ''}`} aria-disabled={missingJustification.length > 0}>
