...
export default async function Dashboard({ params }: PageProps<'/grc/a/[assessmentId]/iso'>) {
  const { assessmentId } = await params;
  const { lang, assessment, customer, methodology, base, customerBase } = await loadAssessment(assessmentId, 'iso27001');
  const BASE = `${base}/iso`;
  const [statusRows, risks] = await Promise.all([getStatusRows(assessment.id), getRisks(customer.id)]);
