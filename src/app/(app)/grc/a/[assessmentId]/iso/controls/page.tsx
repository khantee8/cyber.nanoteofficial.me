...
export default async function ControlsPage(props: PageProps<'/grc/a/[assessmentId]/iso/controls'>) {
  const [{ assessmentId }, params] = await Promise.all([props.params, props.searchParams]);
  const { lang, assessment, base } = await loadAssessment(assessmentId, 'iso27001');
  const BASE = `${base}/iso`;
  const rows = await getStatusRows(assessment.id);
              ...
              {list.map((c) => {
                ...
                return (
                  <tr key={c.id} className="align-top hover:bg-surface-2/60">
                    ...
                    <td className="px-4 py-2 text-right"><StatusSelect assessmentId={assessment.id} controlId={c.id} value={r?.status ?? 'not_started'} lang={lang} /></td>
                  </tr>
                );
              })}
            </tbody>
